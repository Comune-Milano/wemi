...
import { otherPaymentMethodDataTransformer } from './utils/otherPaymentMethodDataTransformer';
import {
...
export default {
  Mutation: {
    otherPaymentMethodTransaction: async (parent, args, context) => {
      const {
      ...
      try {
        resultPayment = await context.db.tx('insert-pagamento', async transaction => {
          ...
          // const { amount } = transactionPayload;
        //   const { transaction: { id: transactionId } } = transactionResult;
          ...
          const jsDatiPagamento = otherPaymentMethodDataTransformer(localtimestamp, totalVoucherImport);
