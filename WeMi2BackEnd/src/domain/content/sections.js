 ...
 */
export class SectionContentDomain extends ContentDomain {
  /**
  ...
  async create(content = {}) {
    ...
    */
  async getSections(parameters) {
    
    const contents = await this.getContentList(parameters);

    const sections = contents.list.map((element) => ({
      ...element, color: element.data?.cd_colore_sezione,
    }));
  
    return {...contents, list: sections};
  }
